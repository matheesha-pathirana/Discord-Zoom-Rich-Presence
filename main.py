...
def default():
    ...
    rpc.update(state="In a Zoom Meeting", large_image="zoom_meeting",large_text="Zoom Meetings",small_image="red_dot",small_text="In a Zoom Meeting", start=time.time())

def run_rpc():
    print("%sZoom has started, connecting Rich Presence!%s" % (fg(46), attr(0)))
    rpc.update(state="In a Zoom Meeting", large_image="zoom_meeting",large_text="Zoom Meetings",small_image="red_dot",small_text="In a Zoom Meeting",details=(topic), start=time.time())
